...
class TrajectoryPlotter(Node):
    def __init__(self):
        ...
        self.csv_dir = os.path.expanduser('~/Mobile_Robot-Lab2/docs')
    ...
    def _start_recording_goto(self, target):
        ...
        fname = f"flight_{timestamp}_GOTO_{target[0]:.1f}_{target[1]:.1f}_{target[2]:.1f}.csv"
    ...
    def _start_recording_mode(self, mode):
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        fname = f"flight_{timestamp}_{mode}.csv"
        self._begin_recording(fname)
